            ...
            if (colliderHit != null)
            {
                if (!alreadyHit.Contains(colliderHit))
                {
                    ...
                    if (colliderHit.CompareTag("treasure"))
                    {
                        ...
                        {
                            print("Equal");
                            pingObj = Instantiate(rPing, colliderHit.transform.position, Quaternion.Euler(90f, 0f, 0f));
                        }
                        else if (colliderHit.transform.position.y < player.transform.position.y)
                        {
                            print("Lower");
                            pingObj = Instantiate(rPing_Low, colliderHit.transform.position, Quaternion.Euler(90f, 0f, 0f));
                        }
                        else if (colliderHit.transform.position.y > player.transform.position.y)
                        {
                            print("Higher");
                            pingObj = Instantiate(rPing_Up, colliderHit.transform.position, Quaternion.Euler(90f, 0f, 0f));
                        }
